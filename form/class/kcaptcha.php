 ...
 * Капча
 *
 * @todo Ну надо это мясо перелопатить :)
 */
class form_kcaptcha extends mod_controller{
    ...
    public function index($p){
        ...
        if (mod::conf("form:black_noise_density") != "") {
            $black_noise_density = 1 / mod::conf("form:black_noise_density"); }
        else {
            $black_noise_density = 1/30; }
            
        if (mod::conf("form:fluctuation_amplitude") != "") {
            $fluctuation_amplitude = mod::conf("form:fluctuation_amplitude"); }
        else {
        ...
        // amplitudes
        if (mod::conf("form:rand9") != "" && mod::conf("form:rand10") != "") {
            $rand9 = mt_rand(mod::conf("form:rand9"),mod::conf("form:rand10"))/110;
            $rand10 = mt_rand(mod::conf("form:rand9"),mod::conf("form:rand10"))/100; }
        else {
        ...
        mod::session($name,$this->getKeyString());
   ...
   public function validate($value,$data,$field) {
        if(isset($_SESSION[$field->name()]) && $_SESSION[$field->name()] === $value){
                return true;
        }else{
                return $field->error();
        }
