<?
	...
	public static function all() { return reflex::get(get_class())->joinByField("domain")->neq("seo_domain.id",0)->asc("priority"); }
	public static function get($id) { return reflex::get(get_class(),$id); }

	public function reflex_title() { return $this->data("query"); }
	public function _domain() { return seo_domain::get($this->data("domain")); }
	...
	public function reflex_repair() { $this->setUpdateTime(); }
	...
	public static function refreshLast() {
	    $now = util::now()->notime()."";
	    $query = self::all()->where("date(`update`)<>'$now' or `update` is null ")->one();
	    if($query->exists())
	        $query->refresh();
	}
