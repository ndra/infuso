 ...
 **/
class reflex_table extends mod_component {
    ...
    /**
     * Готовы ли поля для использования
     * Если не готовы, к ним нужно прицепить поведения, специфические для полей reflex
     * Флаг сбрасывается при изменении таблицы (добавлении поля и т.д.)
     **/
    protected $fieldsReady = false;
    
    /**
     * Массив с группами полей таблицы
     **/
    ...
    /**
     * Флаг, показывающий что поля были инициализированы
     **/
    protected $fieldsWokenUp = false;

    /**
     * Флаг, показывающий что группы полей были инициализированы
     **/
    protected $fieldGroupsWokenUp = false;
    
    /**
     * Флаг, показывающий что индексы
     **/
    protected $indexesWokenUp = false;

    /**
     * Буфер для фабрики таблиц byName()
     * Каждая таблица после создания хранится в буфере и второй раз возьмется оттуда
     **/
    protected static $bufferAll = array();
    ...
    public function factoryTableForReflexClass($class) {
        ...
        if(!self::$bufferReflex[$class]) {
            ...
            $iClass = reflex_util::getItemClass($class);
            ...
            if(is_string($ret)) {
                $table = reflex_table::factoryByName($ret);
            } elseif (is_array($ret)) {
                $table = new self(util::id(),$ret);
            } else {
                $table = new reflex_table(null);
            }
    ...
    public function saveConf() {
        ...
        $data = $this->serialize();
          util::save_for_inclusion($path."",$data);
    }
    ...
    public function fieldsLazyInit() {
        ...
        if($conf = $this->conf["fields"]) {
            foreach($conf as $fieldData) {
                $this->fields[] = mod_field::get($fieldData);
            }
        }
    }
