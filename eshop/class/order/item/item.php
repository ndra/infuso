...
class eshop_order_item extends reflex {
    ...
    public static function deleteAllNonActiveOrderedItems() {
        $items = eshop_order_item::all()->joinByField("itemID")->joinByField("orderID")->where("`eshop_item`.`active`=0 and `eshop_order`.`status`=''");
        $items->delete();    
    }
