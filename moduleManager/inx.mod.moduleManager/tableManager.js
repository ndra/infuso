...
inx.mod.moduleManager.tableManager = inx.list.extend({
    ...
    constructor:function(p) {    
        p.loader = {cmd:"moduleManager_tableManager:listTables",module:p.module};
        p.tbar = [
    ...
    cmd_addTable:function() {
        this.call({cmd:"moduleManager_tableManager:addTable",module:this.module},
        [this.id(),"load"]);
    },
    ...
    cmd_deleteTable:function() {
        ...
        if(!id) return;
        if(!confirm("Удалить таблицу?")) return false;        
        this.call(
            {cmd:"moduleManager_tableManager:deleteTable",id:id,module:this.module},
            [this.id(),"load"]
        );
    },    
